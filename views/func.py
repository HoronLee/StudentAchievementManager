...
class ViewMark:
    ...
    def view_show(entry):
        ...
        if entry.isdigit():
            ...
            flag = 1
        students = load_mark("marks.txt")
        students.insert(0, ['学号', '姓名', '语文', '数学', '英语'])
        if flag == 0:
            out = showByNo(sno, students)
            if len(out) == 0:
                out = [['找不到任何内容', '找不到任何内容', '找不到任何内容', '找不到任何内容', '找不到任何内容']]
            se.output(out)
        elif flag == 1 and entry != '':
            out = showByName(sname, students)
            if len(out) == 0:
                out = [['找不到任何内容', '找不到任何内容', '找不到任何内容', '找不到任何内容', '找不到任何内容']]
            se.output(out)
        else:
